...
function ascend() {
  ...
  for (var i = 0; i < buy_buttons.length; i++) {
    if (buy_buttons[i].innerHTML != "MAXXED") {
      max = false;
    }
  }
  if (answer.toLowerCase() == "yes" && max) {
    ...
    document.querySelectorAll("button.buy").forEach((buttonnn) => {
      ...
      if (buttonnn.id == "11") {
        document.getElementById("6").innerHTML = "Ascended cost:2000 uprate:45";
      }
      if (buttonnn.id == "22") {
        document.getElementById("7").innerHTML =
          "Ascended cost:6000 uprate:150";
      }
      if (buttonnn.id == "33") {
        document.getElementById("8").innerHTML =
          "Ascended cost:20000 uprate:525";
      }
      if (buttonnn.id == "44") {
        document.getElementById("9").innerHTML =
          "Ascended cost:40000 uprate:1275";
      }
      if (buttonnn.id == "55") {
        document.getElementById("10").innerHTML =
          "Ascended cost:100000 uprate:3435";
      }
      if (buttonnn.id == "rate") {
        buttonnn.innerHTML = "Ascended UPGRADE MONEY RATE: 150000";
        buttonnn.onclick = money_rate;
      }
    });
...
function upgrade_labor() {
  if (
    Number(money.innerHTML.slice(6)) >=
    Number(document.getElementsByTagName("label")[0].innerHTML.slice(6))
  ) {
    money.innerHTML =
      "Money: " +
      (Number(money.innerHTML.slice(6)) -
...
function buton_activate() {
  if (event.srcElement.id == "11") {
    buton(15 * mult, 2000, "1");
  }
  if (event.srcElement.id == "22") {
    buton(50 * mult, 6000, "2");
  }
  if (event.srcElement.id == "33") {
    buton(175 * mult, 20000, "3");
  }
  if (event.srcElement.id == "44") {
    buton(425 * mult, 40000, "4");
  }
  if (event.srcElement.id == "55") {
    buton(1145 * mult, 100000, "5");
  }
}
var count = 5;
function buton(upp, cost, id) {
  ...
  if (real_money >= cost) {
    ...
    document.getElementById(id).innerHTML =
      Number(document.getElementById(id).innerHTML) + 1;
  } else {
    alert("Broke nigga");
  }
